...
export default function useTableHooks(){
   ...
   //存放校验错误验证数据
    const errorFieldValues = ref<FieldError[][]>([])
   //formRef
    const  formRef = ref()
   //正在编辑数组的key名
   ...
   const editFlag = ref('')
     ...
     * @param emit
     */
    function dataInit(props: TableProps,emit: Function) {
     ...
     * @param value
     */
    function setTableField(index: number,prop: string,value: any){
     ...
     * @param indexs
     */
    function removeAll(indexs: number[]) {
     ...
     * @param isCoerce
     */
    function setTableRowUpdate(index: number,isCoerce: boolean = false){
